...
def run_react_agent(user_query: str, provider, mcp_server: MCPHRServer) -> list:
    ...
    step = 0
    trace_logs = []
    history = []   # Lịch sử các bước Action -> Observation, nạp lại cho LLM ở lượt kế tiếp
    ...
    while step < MAX_ITERATIONS:
        ...
        # Gọi LLM với Native Tool Calling Specs + toàn bộ Observation của các bước trước
        ...
        # Trường hợp 2: LLM đề xuất gọi Tool (Action)
            ...
            # Nạp Observation vào lịch sử -> quay lại đầu vòng lặp để LLM suy luận tiếp
            history.append({
                "tool_name": tool_name,
                "arguments": arguments,
                "observation": obs_str,
            ...
            print(f"⚠️ [CHÚ Ý]: LLM trả về định dạng không xác định: {llm_response}")
            break
    else:
        # Vượt quá MAX_ITERATIONS mà chưa có Final Answer
        print(f"⛔ [STOP]: Đã đạt giới hạn {MAX_ITERATIONS} bước mà chưa có câu trả lời cuối cùng.")
        trace_logs.append({
            "step": step + 1,
            "query": user_query,
            "action_type": "MAX_ITERATIONS_REACHED",
            "thought": "Vượt quá số bước cho phép.",
            "output": "",
            "latency_ms": 0
        })
...
if __name__ == "__main__":
    print("==========================================================")
    print("🏢 VINFAST HR ASSISTANT - DAY 03 LAB: CHATBOT VS REACT AGENT")
    print("==========================================================")
    ...
    if "--interactive" in sys.argv:
        ...
        print("   - Tra cứu ngày phép: 'Nhân viên VF2026001 còn bao nhiêu ngày phép?'")
        print("   - Tra cứu bảo hiểm: 'Nhân viên VF2026001 đang có gói bảo hiểm gì?'")
        print("   - Tạo đơn nghỉ phép: 'Tạo đơn xin nghỉ phép cho VF2026001 từ 15/09/2026 đến 17/09/2026'")
